...
(function() {
  ...
  module.exports = function(grunt) {
    var append, back_to_top, generate_TOC, generate_banner, generate_footer, generate_release_history, generate_title, get_file_extension, get_latest_changelog, get_package_info, inform, is_valid_extention, make_anchor, parse_changelog, parse_changelogs, sort_changelogs;
    inform = function(msg) {
    ...
    get_latest_changelog = function(opts) {
      var changelog_folder, filename, files, prefix, versions_found, _i, _len;
      if (opts.informative) {
      ...
      if (versions_found.length > 0) {
        return sort_changelogs(parse_changelogs(prefix, versions_found)).slice(-1)[0].filename;
      } else {
        grunt.fail.fatal("No changelogs are present. Please write a changelog file or fix prefixes.");
        return false;
      }
    };
    sort_changelogs = function(changelogs) {
      return changelogs.sort(function(a, b) {
        return semver.compare(a.version, b.version);
      });
    };
    parse_changelogs = function(prefix, filenames) {
      return filenames.map(function(filename) {
        return parse_changelog(prefix, filename);
      });
    };
    parse_changelog = function(prefix, filename) {
      return {
        filename: filename,
        prefix: filename.substring(0, prefix.length),
        version: filename.substring(prefix.length, (filename.length - get_file_extension(filename).length) - 1),
